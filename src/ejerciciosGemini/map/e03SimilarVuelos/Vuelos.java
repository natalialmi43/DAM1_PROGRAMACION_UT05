...
public class Vuelos {
    ...
    public static Double getBilleteMasCaro(Map<String, Map<String, Double>> vuelosPasajerosCoste) {
        ...
        Double precioBilleteMasCaro = 0.0;

        for (Map<String, Double> mapaPasajerosPrecio : vuelosPasajerosCoste.values()){
            for (Double  precioBillete: mapaPasajerosPrecio.values()){
                if (precioBillete > precioBilleteMasCaro){
                    precioBilleteMasCaro = precioBillete;
                }
            }
        }

        return precioBilleteMasCaro;
    }
    ...
    public static Map<String, Double> getPreciosMedios(Map<String, Map<String, Double>> vuelosPasajerosCoste) {
        ...
        for (String idVuelo : vuelosPasajerosCoste.keySet()){
            Map<String, Double> mapaPajeroPrecio = vuelosPasajerosCoste.get(idVuelo);
            Double precioMedioPorVuelo = 0.0;
            for (Double precioBillete : mapaPajeroPrecio.values()){
                precioMedioPorVuelo += precioBillete;
            }
            Double precioMedioCalculado = precioMedioPorVuelo/mapaPajeroPrecio.size();

            precioMedioBilletePorVuelo.put(idVuelo, precioMedioCalculado);
        }
         */

        for(Map.Entry<String, Map<String, Double>> mapaCompleto :vuelosPasajerosCoste.entrySet()){
            String idVuelo = mapaCompleto.getKey();
            Map<String, Double> mapaPasajerosPrecio = mapaCompleto.getValue();

            Double precioTotal = 0.00;
            for (Double precioBillete : mapaPasajerosPrecio.values()){
                precioTotal += precioBillete;
            }
            Double precioMedio = precioTotal/mapaPasajerosPrecio.size();

            precioMedioBilletePorVuelo.put(idVuelo, precioMedio);
        }
    ...
    public static Map<String, Double> getGastoPorDNI(Map<String, Map<String, Double>> vuelosPasajerosCoste) {
        ...
        Map<String, Double>  precioMedioPorDNI = new HashMap<>();

        for (Map<String, Double> mapaPasajeroPrecio : vuelosPasajerosCoste.values()){

            for (Map.Entry<String, Double> mapaParaSacarDatos : mapaPasajeroPrecio.entrySet()){

                String idPersona = mapaParaSacarDatos.getKey();
                Double precioBillete = mapaParaSacarDatos.getValue();
                Double precioAcumulado = precioMedioPorDNI.getOrDefault(idPersona, 0.00);
                Double precioReal = precioAcumulado + precioBillete;

                precioMedioPorDNI.put(idPersona, precioReal);
            }
        }

        return precioMedioPorDNI;
